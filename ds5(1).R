...
Food_Delivery_Times<-dd    
boxplot(Food_Delivery_Times[,7:10])
x = table(Food_Delivery_Times$Time_of_Day )
per = paste0(round(100*x/sum(x)),"%")
pie(x , labels = per ,main = "How the delivery time is dustributed throughout the day",col= c("red","blue","green","pink","purple"))
legend("bottomright", legend = c("Evening", "Afternoon","Night","morning","character"), fill = c("red", "blue","green","pink"))
boxplot( x = Food_Delivery_Times$Delivery_Time_min , main = "the overall spread and central tendency of the total delivery time",
         xlab = "tendency" , ylab = "time (min)")

plot( x = Food_Delivery_Times$Delivery_Time_min , y = Food_Delivery_Times$Distance_km,  xlab = "delivery time (min)" , ylab = "distance (km)" , col = "red")